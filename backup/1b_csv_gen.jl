...
t_generating = @elapsed @distributed for idx in 0:(num_files-1)
    @info("Generating $(idx+1)/$(num_files)")
    generate_csv_file(idx)
end
@info("Finished processing $(length(num_files)) in $(t_generating) seconds ($(t_generating/length(num_files)) per file)")
results = Dict(:num_files => num_files, :time_per_file => t_generating/length(num_files))
...
JuliaHub.upload_dataset(JuliaHub.dataset(DATASET_NAME), output_dir; update=true)
