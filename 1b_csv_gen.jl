...
using CSV, Scratch, DSP, DataFrames, Printf, Distributed
...
const DATASET_USER = "jacob_vaverka2"
const DATASET_NAME = "parallel-computing-webinar-dataset"

csv_dir = @get_scratch!("generated_csvs")
@info("Uploading new version to DataSet: $(DATASET_USER)/$(DATASET_NAME)")
JuliaHub.upload_dataset(JuliaHub.dataset(DATASET_NAME), csv_dir; update=true)
...
begin
    ...
    plt_output = "sine_bundle.png"
    savefig(plt, plt_output)
    ENV["RESULTS_FILE"] = plt_output
end